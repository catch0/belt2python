...
def register(request):
    check = User.userManager.register(request.POST['first_name'], request.POST['alias'], request.POST['email'], request.POST['password'], request.POST['password_confirmation'], request.POST['birthday'])
    if (not check[0]):
        errors = []
...
def profile(request, id):
    ...
    return render(request, 'beltapp/profile.html', context)
...
def unfriend(request, id):
    User.userManager.unfriend(request.session['id'], id)
    return redirect('/friendpage')
